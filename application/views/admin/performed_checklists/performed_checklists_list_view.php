...
<div class="page-wrapper">
	...
	<div class="page-breadcrumb">
		<div class="row">
			<div class="col-12 d-flex no-block align-items-center">
				<h4 class="page-title">Performed CheckLists</h4>
				<div class="ml-auto text-right">
					<nav aria-label="breadcrumb">
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a
										href="<?php echo base_url('admincontrol/dashboard'); ?>">Home</a></li>
							<li class="breadcrumb-item active" aria-current="page">Performed CheckLists</li>
						</ol>
					</nav>
				</div>
			</div>
							...
							<table id="zero_config" class="table table-striped table-bordered">
								...
								?>
									<tr>
										...
										<td><?php echo $recorditem->created_date; ?></td>
										<td>
											<a class="btn btn-outline-danger"
											   onclick="goto_check_performed_checklist(<?php echo $recorditem->cl_p_id; ?>);"
											   href="javascript:;" title="Edit Record">View Performace</a>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Checklist Performance</h5>
				...
				<div class="modal-body">
					...
					<div class="row">
						...
						<a id="print_performed_checklist" target="_blank" class="btn btn-primary float-right mb-2" style="margin-right: 10px;">PRINT</a>
						</div>
						...
						<div class="col-12 col-md-6">
							<div class="form-group row">
								...
								<div class="col-sm-9">
									<input type="date" class="form-control" name="cl_p_date" id="cl_p_date" autocomplete="off" disabled/>
	...
	<script type="text/javascript">
		...
		function goto_check_performed_checklist(cl_p_id) {
			// alert(cl_p_id);
			if (cl_p_id != "") {

				var form_data = new FormData();
				form_data.append("cl_p_id", cl_p_id);

				$.ajax({
					type: "POST",
					url: "<?php echo site_url('admincontrol/PerformedChecklists/get_perform_checklist_items') ?>",
					dataType: 'json',
					data: form_data,
					contentType: false,
					cache: true,
				...
				// 			//console.log(data);
							...
							$('#cl_eq_id').val(data.s_msg.cl_eq_id).change();
							$('#cl_p_date').val(data.s_msg.cl_p_date);
							$('#print_performed_checklist').attr('href','print_performed_checklist_setpdf/'+data.s_msg.cl_p_id);
							// $('#update_cl_name').val(data.s_msg[0].cl_name);
							// $('#update_cl_frequency').val(data.s_msg[0].cl_frequency).change();

							if(data.cl_msg.length!=0){
								// alert(data.s_msg.length);
								$('#perform_checklist_details_table tbody').html('');
								for(k=0;k<data.cl_msg.length;k++){
									$('#perform_checklist_details_table tbody').append('\
									<tr id="row'+(k+1)+'">\
										<input type="hidden" class="perform-hidden-clitem-id" id="perform-hidden-clitem-id'+(k+1)+'" value="'+data.cl_msg[k].cl_pd_cli_id+'">\
										<td>'+(k+1)+'</td>\
										<td>'+data.cl_msg[k].cli_item+'</td>\
										<td>\
											<select class="form-control perform_clitem_value required-perform" id="perform_clitem_value'+(k+1)+'" disabled>\
												<option value="">--select--</option>\
												<option value="P" '+(data.cl_msg[k].cl_pd_cli_value=='P' ? 'selected' : '')+'>Pass</option>\
												<option value="F" '+(data.cl_msg[k].cl_pd_cli_value=='F' ? 'selected' : '')+'>Fail</option>\
												<option value="N" '+(data.cl_msg[k].cl_pd_cli_value=='N' ? 'selected' : '')+'>NA</option>\
											</select>\
											<small class="invalid-feedback perform_clitem_value'+(k+1)+'"></small>\
										</td>\
										<td>\
											<textarea class="form-control perform_clitem_notes required-perform" id="perform_clitem_notes'+(k+1)+'" disabled>'+data.cl_msg[k].cl_pd_cli_notes+'</textarea>\
											<small class="invalid-feedback perform_clitem_notes'+(k+1)+'"></small>\
										</td>\
										<td>\
										'+(data.cl_msg[k].cl_pd_cli_attachment=='' ? 'Not Attached' : '<img style="width:100px" src="<?php echo site_url('upload_file/checklist_perform/') ?>'+data.cl_msg[k].cl_pd_cli_attachment+'">')+'\
										</td>\
									</tr>\
									');
								}
