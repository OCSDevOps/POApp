							    ...
							    <div class="card-body">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-4">
											...
											<input type="hidden" name="itemdtl_qty" id="itemdtl_qty" value="0" autocomplete="off" />
                                            <input type="text" class="form-control" name="pkg_name" id="pkg_name" placeholder="Enter Package Name" autocomplete="off" />
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-8">
                                            <textarea class="form-control" name="pkg_detail" id="pkg_detail" placeholder="Enter Package Details" autocomplete="off"></textarea>
									...
									<div class="form-group row justify-content-md-center">
										<div class="col-sm-10">
											...
											<table width="100%" class="table table-bordered" id="package-items-table">
												<thead>
													<tr>
														<th>Item</th>
														<th>CostCode</th>
												...
												<tr>
													<td>
														<select class="form-control select2 custom-select pk_item package-item-required" name="pk_item" id="pk_item1" data-live-search="true" autocomplete="off" onchange="goto_check_item(1);">
														...
														<small class="invalid-feedback pk_item1"><?php echo form_error('pk_item1'); ?></small>
													</td>
													<td>
														<input type="text" class="form-control pk_ccode package-item-required" name="pk_ccode" id="pk_ccode1" placeholder="Item Cost Code" autocomplete="off" readonly />
														<small class="invalid-feedback pk_ccode1"><?php echo form_error('pk_ccode1'); ?></small>
													</td>
													<td>
														<input type="text" class="form-control pk_itm_qnty package-item-required" name="pk_itm_qnty" id="pk_itm_qnty1" placeholder="Enter Quantity" autocomplete="off" />
														<small class="invalid-feedback pk_itm_qnty1"><?php echo form_error('pk_itm_qnty1'); ?></small>
													</td>
													<!-- <td>
...
<script type="text/javascript">
	...
	function gotoclclickbutton(){
		...
		if(pkg_name == ""){
			e_error = 1;
			$('.pkg_name').html('Package Name is Required.');
		}else{
		...
		$('.package-item-required').each(function(){
			var id=$(this).attr('id');
			if ($('#'+id).val() == "") {
				e_error = 1;
				$('.'+id).html('Required');
		...
		if(e_error == 1){
			...
			var tableRow=0;
			for(i=1;i<=$('#package-items-table tbody tr').length;i++){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('admincontrol/packages/new_package_submission') ?>",
				dataType: 'json',
				data: form_data,
				contentType:false,
				cache:true,
	...
	function getItemMasterItemsdata(){
		...
		function goto_update_add_item(){
			rowCountUpdate=($('#package-items-table tbody tr').length+1);
			$('#package-items-table tbody').append('\
			<tr id="row'+rowCountUpdate+'">\
				<input type="hidden" class="hidden-package-item-id" id="hidden-package-item-id'+rowCountUpdate+'">\
				<td>\
					<select class="form-control select2 custom-select package-item-required pk_item" name="pk_item" id="pk_item'+rowCountUpdate+'" data-live-search="true" autocomplete="off" onchange="goto_check_item('+rowCountUpdate+');">\
						<option value="">---Select---</option>\
						<?php foreach($itm_list as $items){ ?>
							<option value="<?php echo $items->item_code; ?>"><?php echo htmlspecialchars($items->item_code . ' - ' . $items->item_name, ENT_QUOTES); ?></option>\
						<?php } ?>
				...
				<td>\
					<input type="text" class="form-control package-item-required pk_ccode" name="pk_ccode" id="pk_ccode'+rowCountUpdate+'" placeholder="Item Cost Code" autocomplete="off" readonly />\
					<small class="invalid-feedback pk_ccode'+rowCountUpdate+'"></small>\
				</td>\
				<td>\
					<input type="text" class="form-control package-item-required pk_itm_qnty" name="pk_itm_qnty" id="pk_itm_qnty'+rowCountUpdate+'" placeholder="Enter Quantity" autocomplete="off" />\
		...
		$('#package-items-table tbody').on('click', '#removeItem', function(){
			if (confirm('Are you sure you want to delete ?')) {
				var rowId=$(this).closest('tr').attr('id');
				var index=rowId.match(/\d+/);
				if($(this).closest('tr').children('#hidden-package-item-id'+index).val()!=""){
					rmComponentArray1.push($(this).closest('tr').children('#hidden-package-item-id'+index).val());
				}
				...
				child.each(function(){
					...
					$(this).children('.hidden-package-item-id').attr('id',`hidden-package-item-id${dig-1}`);
			...
			// 	if($(this).is(':checked')){
				for(itemCount=0;itemCount<itemIds.length;itemCount++){
					if($('#pk_item'+itemCount).val()==''){
						...
						var adItemCount=($('#package-items-table tbody tr').length+1);
						$('#package-items-table tbody').append('\
						<tr id="row'+adItemCount+'">\
							<input type="hidden" class="hidden-package-item-id" id="hidden-package-item-id'+adItemCount+'">\
							<td>\
								<select class="form-control select2 custom-select package-item-required pk_item" name="pk_item" id="pk_item'+adItemCount+'" data-live-search="true" autocomplete="off" onchange="goto_check_item('+adItemCount+');">\
									<option value="">---Select---</option>\
									<?php foreach($itm_list as $items){ ?>
										<option value="<?php echo $items->item_code; ?>"><?php echo htmlspecialchars($items->item_code . ' - ' . $items->item_name, ENT_QUOTES); ?></option>\
									<?php } ?>
							...
							<td>\
								<input type="text" class="form-control package-item-required pk_ccode" name="pk_ccode" id="pk_ccode'+adItemCount+'" placeholder="Item Cost Code" autocomplete="off" readonly />\
								<small class="invalid-feedback pk_ccode'+adItemCount+'"></small>\
							</td>\
							<td>\
								<input type="text" class="form-control package-item-required pk_itm_qnty" name="pk_itm_qnty" id="pk_itm_qnty'+adItemCount+'" placeholder="Enter Quantity" autocomplete="off" />\
