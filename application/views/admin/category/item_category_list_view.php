...
<?php $this->load->view('admin/component/menu') ?>
                    ...
                    <div class="col-12">
						<div class="card">
                            <div class="card-body">
								...
								<?php 
								if($this->session->userdata('utype')==1 || $templateDetails->pt_m_item<3){?>
									<a href="javascript:;" onclick="goto_add_record();" class="btn btn-primary float-right mb-2" style="margin-right: 10px;">Add New Item Category</a>
									<a href="javascript:;" onclick="goto_bulkupload_record();" class="btn btn-primary float-right mb-2" style="margin-right: 10px;">Bulk upload - Category</a>
								<?php }?>
                                <div class="table-responsive">
                                    <table id="zero_config" class="table table-striped table-bordered">
										...
										<tr>
											...
											<td><?php if($recorditem->icat_status == 1){ ?>
												  <span style="color:green;">Active</span>
											  <?php }elseif($recorditem->icat_status == 0){ ?>
												<span style="color:red;">InActive</span>
											  <?php } ?></td>
											<?php 
											if($this->session->userdata('utype')==1 || $templateDetails->pt_m_item<3){?>
											<td>
												<a class="btn btn-outline-warning" onclick="modify_record(<?php echo $recorditem->icat_id; ?>);" href="javascript:;<?php //echo base_url().'admincontrol/itemcategory/edit_user/'.$recorditem->icat_id; ?>" title="Edit Record"><i class="fa fa-edit text-primary"></i></a>
												<?php if($recorditem->icat_status == 1){ ?>	
												<a class="btn btn-outline-warning" href="<?php echo base_url().'admincontrol/itemcategory/lock_item_category_set/'.$recorditem->icat_id; ?>" title="Lock Record"><i class="fa fa-unlock text-dark"></i></a>
												<?php } else { ?>
												<a class="btn btn-outline-warning" href="<?php echo base_url().'admincontrol/itemcategory/unlock_item_category_set/'.$recorditem->icat_id; ?>" title="Unock Record"><i class="fa fa-lock text-dark"></i></a>
												<?php } ?>
												<?php 
												if($this->session->userdata('utype')==1 || $templateDetails->pt_m_item<2){?>
													<a class="btn btn-outline-warning" onclick="return confirm('You are about to delete a record. This cannot be undone. Are you sure?');" href="<?php echo base_url().'admincontrol/itemcategory/delete_itemset/'.$recorditem->icat_id; ?>" title="Delete Record"><i class="fa fa-trash text-danger"></i></a>
												<?php }?>
											</td>
											<?php }?>
										</tr>	
		...
		<div class="modal-dialog modal-md" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Add New Item Category</h5>
				...
				<div class="modal-body">
						<div class="form-group">
							...
							<input type="text" class="form-control" placeholder="Enter Name" name="name_cat" id="name_cat" autocomplete="off" />
							<small class="invalid-feedback name_cat"><?php //echo form_error('name_cat'); ?></small>
						</div>
						<!--<div class="form-group">
		...
		<div class="modal-dialog modal-md" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
						<div class="form-group">
							<label for="recipient-name" class="col-form-label">Name of Category:</label>
							<input type="hidden" name="update_id_cat" id="update_id_cat" value="" autocomplete="off" />
							<input type="text" class="form-control" placeholder="Enter Name" name="update_name_cat" id="update_name_cat" autocomplete="off" />
							<small class="invalid-feedback update_name_cat"><?php //echo form_error('update_name_cat'); ?></small>
						</div>
						<!--<div class="form-group">
							<label for="message-text" class="col-form-label">Message:</label>
							<textarea class="form-control" id="message-text"></textarea>
	...
	<div class="modal fade" id="Modal_bulkupload_record" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true" data-backdrop="static" data-keyboard="false">
		<div class="modal-dialog modal-md" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Bulk upload - Item Category</h5>
...
<script type="text/javascript">
	...
	function goto_submit_record(){
		...
		if(name_cat == ""){
			e_error = 1;
			$('.name_cat').html('Category Name is Required.');
		}else{
			if(!name_cat.match(alphanumerics_spaces)){
				e_error = 1;
				$('.name_cat').html('Category Name not use special carecters [without _ . , -], Check again.');
			}else{
				$('.name_cat').html('');
			}	
		}
		if (e_error == 1) {
			...
			form_data.append("name_cat", name_cat);
			
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('admincontrol/itemcategory/add_new_item_category') ?>",
				dataType: 'json',
				data: form_data,
				contentType:false,
				cache:true,
				...
				success:function(data){
					...
					if(data.msg == 1)
					{
						...
						setTimeout(function(){ 
							window.location.replace("<?php echo site_url('admincontrol/itemcategory/item_category_list') ?>");
						}, 2000);
	...
	function modify_record(element){
		...
		if(element != ""){
			var form_data = new FormData();
			form_data.append("name_catid", element);
			
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('admincontrol/itemcategory/get_details_of_category') ?>",
				dataType: 'json',
				data: form_data,
				contentType:false,
				cache:true,
				...
				success:function(data){
					...
					if(data.msg == 1)
					{
						//console.log(data);
						//alert(data.msg[0].space_rate);
						$('#update_id_cat').val(element);
						$('#update_name_cat').val(data.s_msg.icat_name);
	...
	function goto_update_record(){
		...
		var update_id_cat = $('#update_id_cat').val();
		var update_name_cat = $('#update_name_cat').val();
		...
		if(update_name_cat == ""){
			e_error = 1;
			$('.update_name_cat').html('Category Name is Required.');
		}else{
			if(!update_name_cat.match(alphanumerics_spaces)){
				e_error = 1;
				$('.update_name_cat').html('Category Name not use special carecters [without _ . , -], Check again.');
			}else{
				$('.update_name_cat').html('');
			}	
		}
		if (e_error == 1) {
			...
			var form_data = new FormData();
			form_data.append("update_id_cat", update_id_cat);
			form_data.append("update_name_cat", update_name_cat);
			
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('admincontrol/itemcategory/modify_item_categorysets') ?>",
				dataType: 'json',
				data: form_data,
				contentType:false,
				cache:true,
	...
	function goto_upload_setof_record(){
		...
		if (e_error == 1) {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('admincontrol/itemcategory/bulkitem_upload_section_sets') ?>",
				dataType: 'json',
				data: form_data,
				contentType:false,
				cache:true,
