...
@section('title', 'Create Cost Code Template')
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h5 class="mb-0 font-weight-bold text-primary">
                <i class="fas fa-layer-group me-1"></i> Create Cost Code Template
            </h5>
            <small class="text-muted">Select cost codes from the standard list to build a reusable template</small>
        </div>
    ...
    <form method="POST" action="{{ route('admin.costcode-templates.store') }}" id="templateForm">
        @csrf
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('cct_name') is-invalid @enderror"
                                   id="cct_name" name="cct_name" value="{{ old('cct_name') }}"
                                   placeholder="e.g. Townhome Cost Codes" required>
                            @error('cct_name')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="cct_description" name="cct_description"
                                      rows="3" placeholder="Brief description of when to use this template">{{ old('cct_description') }}</textarea>
                        </div>
                            ...
                            @foreach($parentCodes as $parent)
                                <div class="cost-code-group mb-2" data-search="{{ strtolower($parent->cc_full_code . ' ' . $parent->cc_description) }}">
                                    <div class="form-check border-bottom pb-2 mb-1">
                                        <input class="form-check-input cost-code-checkbox parent-checkbox" type="checkbox"
                                               name="cost_code_ids[]" value="{{ $parent->cc_id }}"
                                               id="cc_{{ $parent->cc_id }}"
                                               data-code="{{ $parent->cc_full_code }}"
                                               {{ in_array($parent->cc_id, old('cost_code_ids', [])) ? 'checked' : '' }}>
                                    ...
                                    @foreach($categoryCodes->where('cc_parent_code', $parent->cc_no) as $category)
                                        <div class="ms-4 cost-code-item" data-search="{{ strtolower($category->cc_full_code . ' ' . $category->cc_description) }}">
                                            <div class="form-check mb-1">
                                                <input class="form-check-input cost-code-checkbox category-checkbox" type="checkbox"
                                                       name="cost_code_ids[]" value="{{ $category->cc_id }}"
                                                       id="cc_{{ $category->cc_id }}"
                                                       data-parent="{{ $parent->cc_no }}"
                                                       {{ in_array($category->cc_id, old('cost_code_ids', [])) ? 'checked' : '' }}>
                                                ...
                                                <div class="ms-4 cost-code-item" data-search="{{ strtolower($sub->cc_full_code . ' ' . $sub->cc_description) }}">
                                                    <div class="form-check mb-1">
                                                        <input class="form-check-input cost-code-checkbox" type="checkbox"
                                                               ...
                                                               id="cc_{{ $sub->cc_id }}"
                                                               {{ in_array($sub->cc_id, old('cost_code_ids', [])) ? 'checked' : '' }}>
