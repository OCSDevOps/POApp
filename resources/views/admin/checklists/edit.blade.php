        ...
        <form method="POST" action="{{ route('admin.checklists.update', $checklist) }}">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="cl_name" class="form-control" value="{{ $checklist->cl_name }}" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Frequency</label>
                    <input type="text" name="cl_frequency" class="form-control" value="{{ $checklist->cl_frequency }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Start Date</label>
                    <input type="date" name="cl_start_date" class="form-control" value="{{ $checklist->cl_start_date }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Equipments</label>
                    <select name="cl_eq_ids[]" class="form-select" multiple>
                        @foreach($equipments as $eq)
                            <option value="{{ $eq->eq_id }}" @selected(in_array($eq->eq_id, $checklist->cl_eq_ids ?? []))>{{ $eq->eqm_asset_name ?? ('#'.$eq->eq_id) }}</option>
                        @endforeach
                    </select>
                </div>
                    <div class="col-md-6">
                        ...
                        <select name="cl_user_ids[]" class="form-select" multiple>
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" @selected(in_array($user->id, $checklist->cl_user_ids ?? []))>{{ $user->name ?? $user->email }}</option>
                            @endforeach
                        </select>
                    </div>
            </div>
            ...
            <div id="items-wrapper">
                @foreach($checklist->items as $item)
                    <div class="row g-2 mb-2 item-row">
                        <div class="col-11">
                            <input type="text" name="items[]" class="form-control" value="{{ $item->cli_item }}" required>
                        </div>
