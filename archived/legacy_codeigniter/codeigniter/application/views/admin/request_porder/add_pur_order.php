		...
		<div class="row">
			...
			<textarea class="form-control" name="po_dl_note" id="po_dl_note"
											...
											<td>
												<select class="form-control select2 custom-select pk_item"
													...
													<?php foreach ($itm_list as $items) { ?>
														<option
																value="<?php echo $items->item_code; ?>"><?php echo $items->item_code . ' - ' . $items->item_name; ?></option>
													<?php } ?>
												</select>
											</td>
											<td>
					...
					<div class="border-top">
						<div class="card-body">
							<button type="button" onclick="gotoclclickbutton();" class="btn btn-primary">Add Material
								PO
							</button>
	...
	<script type="text/javascript">
		...
		function goto_check_item_amounts(element) {
			...
			if (pk_itm_qnty != "" && pk_itm_price != "" && pk_taxcode != "") {
				var tax_amount = (parseFloat(subtotal_amount) * parseFloat(pk_taxcode)) / 100;
			} else {
				var tax_amount = 0;
			}
					...
					$.ajax({
						...
						success: function (data) {
							...
							if (data.msg == 1) {
								...
								$('[name="pk_taxcode[]"]').eq(row_id - 1).val('<?php isset($taxcode_set->tc_tax_value) ? $taxcode_set->tc_tax_value : 0 ?>');
		...
		function goto_addTax(element) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data.msg == 1) {
						var pre_tax_amt = $('[name="pre_tax_amt[]"]').eq(row_id - 1).val();
						var tax_amount = parseFloat(pre_tax_amt) * data.taxgroup.percentage / 100;
