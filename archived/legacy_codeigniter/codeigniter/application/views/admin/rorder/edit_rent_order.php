	...
	<div class="container-fluid">
		...
		<div class="row">
				...
				<div class="card">
					...
					<div class="border-bottom">
						<div class="card-body">
							<div class="row">
								<div class="col-md-12">
									<?php if($view) { ?>
									<a href="<?php echo base_url() . 'admincontrol/rorder/print_rorder_setpdf/' . $porder_list->porder_id; ?>"
					...
					<div class="card-body">
						...
						<div class="form-group row">
							...
							<div class="col-sm-3">
								<input type="hidden" name="ipack_itm_no" id="ipack_itm_no" value="<?php echo $at_no; ?>"
									   autocomplete="off"/>
								<input type="hidden" name="itemdtl_counter" id="itemdtl_counter"
									   value="<?php echo $porder_list->porder_total_item; ?>" autocomplete="off"/>
								<input type="hidden" name="itemdtl_tamount" id="itemdtl_tamount"
									   value="<?php echo $porder_list->porder_total_amount; ?>" autocomplete="off"/>
								<input type="hidden" name="po_id" id="po_id"
									   value="<?php echo $porder_list->porder_id ?>">
								...
								<select class="form-control select2 custom-select" name="po_project" id="po_project"
									...
									<?php foreach ($proj_list as $p_items) { ?>
										<option
												value="<?php echo $p_items->proj_id; ?>" <?php if ($p_items->proj_id == $porder_list->porder_project_ms) {
											echo "selected";
										} ?>><?php echo $p_items->proj_name; ?></option>
									<?php } ?>
								</select>
								<small
							...
							<div class="col-sm-3">
								<div class="row">
									...
									<input type="text" class="form-control col-5" readonly name="po_number_prefix"
										   ...
										   autocomplete="off"/>
									<input type="text" class="form-control col-5" name="po_number"
										   value="<?php echo $order_number[1] ?>" id="po_number"
										   autocomplete="off"/>
									<small
							...
							<div class="col-sm-3">
								<select class="form-control select2 custom-select" name="po_supp" id="po_supp"
									...
									<?php foreach ($supp_list as $sup_items) { ?>
										<option
												value="<?php echo $sup_items->sup_id; ?>" <?php if ($sup_items->sup_id == $porder_list->porder_supplier_ms) {
											echo "selected";
										} ?>><?php echo $sup_items->sup_name; ?></option>
									<?php } ?>
								</select>
			...
			<textarea class="form-control" name="po_desc" id="po_desc"
					  ...
					  autocomplete="off"><?php echo $porder_list->porder_description ?></textarea>
								<small
			...
			<textarea class="form-control" name="po_address" id="po_address"
					  ...
					  autocomplete="off"><?php echo $porder_list->porder_address; ?></textarea>
								<small
			...
			<textarea class="form-control" name="po_dl_note" id="po_dl_note"
					  ...
					  autocomplete="off"><?php echo $porder_list->porder_delivery_note; ?></textarea>
								<small
						...
						<div class="form-group row">
							...
							<div class="col-sm-3">
								<input class="form-control" type="date"
									   value="<?php echo date('Y-m-d', strtotime($porder_list->porder_delivery_date)) ?>"
									   name="po_delivery_date" id="po_delivery_date"
							...
							<div class="col-sm-3">
								<input class="form-control" type="text" disabled
									   value="<?php echo $porder_list->porder_general_status ?>"
									...
									<table width="100%" class="table table-bordered" id="porder_table">
										...
										<?php foreach ($item_detailsets as $key => $item) {
											...
											<tr data-id="<?php echo $key + 1 ?>">
													<input type="hidden" name="po_detail_autogen[]" id="po_detail_autogen"
													   value="<?php echo $item->po_detail_autogen ?>">
												<td>
													<select class="form-control select2 custom-select pk_item"
														...
														<?php foreach ($itm_list as $items) { ?>
															<option
																	value="<?php echo $items->item_code; ?>" <?php echo $item->po_detail_item == $items->item_code ? 'selected' : '' ?>><?php echo $items->item_code . ' - ' . $items->item_name; ?></option>
														<?php } ?>
													</select>
												</td>
												<td>
												...
												<td>
													<input type="number" class="form-control pk_itm_qnty"
														   name="pk_itm_qnty[]"
														   value="<?php echo $item->po_detail_quantity ?>"
														   id="pk_itm_qnty" placeholder="Enter Quantity"
														   autocomplete="off"
												...
												<td>
													...
													<input type="hidden" class="form-control pk_taxcode"
														   name="pk_taxcode[]"
														   id="pk_taxcode" placeholder="Enter TaxCode"
														   value="<?php echo $item->po_detail_taxcode ?>"
														   autocomplete="off"
														   readonly/>
													<input type="hidden" class="form-control pk_total_amt"
														   name="pk_total_amt[]"
														   value="<?php echo $item->po_detail_total ?>"
														   id="pk_total_amt" placeholder="Item Total Amount"
														   autocomplete="off" readonly/>
													<input type="hidden" class="form-control pk_subtotal"
														   value="<?php echo $item->po_detail_subtotal ?>"
														   name="pk_subtotal[]"
														   id="pk_subtotal" placeholder="Item Sub Total"
														   autocomplete="off"
														   readonly/>
													<input type="number" class="form-control pk_itm_price"
														   value="<?php echo $item->po_detail_unitprice ?>"
														   name="pk_itm_price[]"
														   id="pk_itm_price" placeholder="Enter Price"
														   data-id="<?php echo $key + 1 ?>"
														   autocomplete="off"
														   onchange="goto_check_item_amounts(this);"/>
													<small
												...
												<td>
													<input type="number" class="form-control pk_rent_duration" data-id="<?php echo $key + 1 ?>"
														   name="pk_rent_duration[]" onchange="goto_check_item_amounts(this);"
														   id="pk_rent_duration" placeholder="Duration in days"
														   value="<?php echo $item->po_detail_duration ?>"
														   autocomplete="off"
													/>
													<small
															class="invalid-feedback "><?php echo form_error('pk_rent_duration'); ?></small>
												</td>
												<td>
												...
												<td>
													<input type="text" class="form-control pre_tax_amt"
														   name="pre_tax_amt[]"
														   value="<?php echo $item->po_detail_subtotal ?>"
														   id="pre_tax_amt" placeholder="Pre-Tax Amount"
														   autocomplete="off"
														   readonly/>
													<small
												...
												<td>
													<input type="text" class="form-control pk_tax_amt"
														   name="pk_tax_amt[]"
														   value="<?php echo $item->po_detail_taxamount ?>"
														   id="pk_tax_amt" placeholder="Item Tax Amount"
														   autocomplete="off"
														   readonly/>
													<small
									...
									<table width="100%" class="table table-bordered">
										...
										<tr>
											...
											<td><span id="pk_sub_total">
												...
												echo $formatter->formatCurrency($item->po_detail_subtotal, 'USD'); 
												?></span>
											</td>
										</tr>
										<tr>
											<th>Total Tax</th>
											<td><span id="pk_total_tax"><?php echo $formatter->formatCurrency($item->po_detail_taxamount, 'USD'); ?></span>
											</td>
										</tr>
										<tr>
											<th>PO Total</th>
											<td><span id="total_po"><?php echo $formatter->formatCurrency($item->po_detail_total, 'USD'); ?></span></td>
										</tr>
										</tbody>
									</table>
								</div>
					...
					<div class="border-top">
						<div class="card-body">
							<button type="button" onclick="gotoclclickbutton('pending');" class="btn btn-primary">
								Update
							</button>
							<button type="button" onclick="gotoclclickbutton('submitted');" class="btn btn-primary">
								Send
							</button>
...
//form_data.append("files", files[0]);
		...
		jQuery(document).delegate('a.add_row', 'click', function (e) {
			...
			var content = '<tr data-id="' + size + '">\n' +
					...
					'\t\t\t\t\t\t<option value="">---Select---</option>\n' +
					'\t\t\t\t\t\t<?php foreach ($itm_list as $items) { ?>\n' +
					'\t\t\t\t\t\t<option value="<?php echo $items->item_code; ?>"> <?php echo $items->item_code . " " . $items->item_name; ?></option>\n' +
					'\t\t\t\t\t\t<?php } ?>\n' +
					'\t\t\t\t\t</select>\n' +
					'\t\t\t\t</td>\n' +
					'<td>\n'+
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (data.msg == 1) {
							//console.log(data);
							//alert(data.msg[0].space_rate);
							var mixstring = '<div class="col-sm-12"><input type="hidden" name="totalitem_pkg" id="totalitem_pkg" value="' + data.s_msg.length + '" /><table class="table"><thead><tr><th>Item Name</th><th>Item Code</th><th>Item Quantity</th><th>Item Price</th></tr></thead><tbody>';
							var ii = 0;
							for (ii = 0; ii < data.s_msg.length; ii++) {
								mixstring = mixstring + '<tr><td>' + data.s_msg[ii].item_name + '</td><td>' + data.s_msg[ii].ipdetail_item_ms + '</td><td><input type="hidden" name="ipackitem_code_' + ii + '" id="ipackitem_code_' + ii + '" value="' + data.s_msg[ii].ipdetail_item_ms + '" /><input type="text" name="ipackitem_qty_' + ii + '" id="ipackitem_qty_' + ii + '" value="' + data.s_msg[ii].ipdetail_quantity + '" /></td><td><input type="text" name="ipackitem_price_' + ii + '" id="ipackitem_price_' + ii + '" /></td></tr>';
							}
						...
						if (data.msg == 1) {
							...
							data.s_msg.forEach(function (item) {
								...
								var content = '<tr data-id="' + size + '">\n' +
										...
										'\t\t\t</tr>';
								$('#porder_table_body').append(content);
								$('#porder_table_body tr[data-id=' + size + ']>td:first').find('select option[value=' + item.po_detail_item + ']').attr('selected', 'selected');
...
//var ap_quaran = $("input[name='ap_quaran']:checked").val();
			...
			var pk_itm_price = $('[name="pk_itm_price[]"]').map(function () {
				return this.value; // $(this).val()
			}).get();
			...
			if (po_number == "") {
				e_error = 1;
				$('.po_number').html('Rental Order No. is Required.');
			} else {
				if (!po_number.match(alphanumerics_spaces)) {
					e_error = 1;
					$('.po_number').html('Rental Order No. not use special carecters [without _ . , -], Check again.');
				} else {
					$('.po_number').html('');
				}
			}
...
//alert(pr_user);return;
			if (e_error == 1) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo site_url('admincontrol/rorder/modify_rorder_set_submission') ?>",
					dataType: 'json',
					data: form_data,
					contentType: false,
					cache: true,
